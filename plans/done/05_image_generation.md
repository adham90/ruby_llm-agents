...
Image generation pricing is complex:
- **RubyLLM/models.dev**: Has model capabilities but **no per-image pricing** (all `nil` or empty)
- **OpenAI/Replicate APIs**: No programmatic pricing endpoints
- **Pricing varies by**: model, size, quality, provider
...
```
1. LiteLLM JSON (primary)     → Comprehensive per-image pricing, community-maintained
2. Configurable pricing table → User overrides and custom models
3. Hardcoded fallbacks        → Last resort estimates
```

### Data Sources Comparison

| Source | Image Models | Per-Image Pricing | Update Frequency |
|--------|--------------|-------------------|------------------|
| **LiteLLM JSON** | ✅ 50+ | ✅ Yes | Daily (community) |
| **models.dev** | ✅ 50+ | ❌ All `nil` | Frequent |
| **RubyLLM** | ⚠️ Limited | ❌ Empty/token-based | With gem updates |
| **OpenRouter API** | ⚠️ Limited | ⚠️ Token-based | Real-time |

### LiteLLM Pricing Data

LiteLLM maintains comprehensive per-image pricing at:
```
https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json
```

**Sample pricing from LiteLLM:**
| Model | Field | Price |
|-------|-------|-------|
| `dall-e-2` (1024x1024) | `input_cost_per_pixel` | $0.02/image |
| `dall-e-3` | `input_cost_per_image` | $0.04-$0.12/image |
| `aiml/flux-pro` | `input_cost_per_image` | $0.053/image |
| `aiml/flux/schnell` | `input_cost_per_image` | $0.003/image |
| `stability.stable-diffusion-xl-v1` | `input_cost_per_image` | $0.04-$0.08/image |

### Implementation

**File: `lib/ruby_llm/agents/image_generator/pricing.rb`**
    ...
    class ImageGenerator
      ...
      module Pricing
        ...
        def calculate_cost(model_id:, size: nil, quality: nil, count: 1)
          cost_per_image = cost_per_image(model_id, size: size, quality: quality)
          (cost_per_image * count).round(6)
        end
        ...
        def cache_ttl
          config.litellm_pricing_cache_ttl || DEFAULT_CACHE_TTL
        end
        ...
        def from_config(model_id, size, quality)
          ...
          pricing = table[model_id] || table[normalized]
        ...
        def resolve_config_price(pricing, size, quality)
          ...
          # Base price
          pricing[:base] || pricing["base"] || pricing[:default] || pricing["default"]
        end
...
```ruby
RubyLLM::Agents.configure do |config|
  # Cache TTL for LiteLLM pricing data (default: 24 hours)
  config.litellm_pricing_cache_ttl = 24.hours

  # Default cost when model not found in any source
  config.default_image_cost = 0.04

  # Custom/override pricing table (takes precedence over LiteLLM)
  config.image_model_pricing = {
    ...
    "gpt-image-1" => {
      base: 0.04,        # standard quality
      hd: 0.08,          # HD quality
      large_hd: 0.12     # HD + large size (1792x1024)
    }
  }
end
```

### Usage in Result Class
...
RubyLLM::Agents::ImageGenerator::Pricing.refresh!

# View all pricing sources
RubyLLM::Agents::ImageGenerator::Pricing.all_pricing
# => {
#   litellm: { "dall-e-3" => {...}, "flux-pro" => {...}, ... },
#   configured: { "my-custom-model" => 0.05 },
#   fallbacks: { "gpt-image-1" => {...}, ... }
# }

# Get price for specific model
RubyLLM::Agents::ImageGenerator::Pricing.cost_per_image(
  "gpt-image-1",
  size: "1024x1024",
  quality: "hd"
)
...
### Rake Task for Price Updates

```ruby
# lib/tasks/image_pricing.rake
namespace :ruby_llm_agents do
  namespace :pricing do
    desc "Refresh image model pricing from LiteLLM"
    task refresh: :environment do
      puts "Fetching latest pricing from LiteLLM..."
      data = RubyLLM::Agents::ImageGenerator::Pricing.refresh!

      image_models = data.select { |k, v| v["input_cost_per_image"] }
      puts "Found #{image_models.size} image models with pricing"

      image_models.first(10).each do |model, pricing|
        cost = pricing["input_cost_per_image"]
        puts "  #{model}: $#{cost}/image"
      end
    end

    desc "Show current image pricing"
    task show: :environment do
      pricing = RubyLLM::Agents::ImageGenerator::Pricing.all_pricing

      puts "\n=== LiteLLM Pricing (#{pricing[:litellm].size} models) ==="
      pricing[:litellm].first(10).each do |model, data|
        cost = data["input_cost_per_image"] || "varies"
        puts "  #{model}: $#{cost}"
      end

      puts "\n=== Configured Overrides ==="
      pricing[:configured].each do |model, cost|
        puts "  #{model}: #{cost.inspect}"
      end
    end
  end
end
...
attr_accessor :litellm_pricing_url          # URL for LiteLLM pricing JSON
attr_accessor :litellm_pricing_cache_ttl    # Cache TTL for pricing data
attr_accessor :default_image_cost           # Fallback cost when pricing unavailable
attr_accessor :image_model_pricing          # Custom pricing overrides per model
...
def initialize
  ...
  # Pricing Configuration
  @litellm_pricing_url = "https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json"
  @litellm_pricing_cache_ttl = 24.hours
  @default_image_cost = 0.02  # Conservative fallback per image
  @image_model_pricing = {}   # Override pricing: { "model-id" => { "1024x1024" => 0.04 } }
end
```
...
```ruby
RubyLLM::Agents.configure do |config|
  # Basic image defaults
  config.default_image_model = "gpt-image-1"
  config.default_image_size = "1024x1024"
  config.default_image_quality = "hd"

  # Pricing configuration
  config.litellm_pricing_cache_ttl = 12.hours  # Refresh pricing every 12 hours
  config.default_image_cost = 0.03             # Fallback cost per image

  # Override pricing for specific models (takes precedence over LiteLLM)
  config.image_model_pricing = {
    "dall-e-3" => {
    ...
    "custom-model" => {
      "default" => 0.10  # Your custom model pricing
    }
  }
end
```
...
- **LiteLLM Pricing JSON** (for dynamic per-image pricing)
  - URL: `https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json`
  - Purpose: Dynamic cost calculation for image models
  - Fallback: Configuration overrides or hardcoded estimates
...
- **Pricing is dynamic**: Uses LiteLLM JSON for per-image costs with configuration overrides and fallbacks (see Dynamic Pricing Strategy section)
- Some features (ControlNet, advanced editing) may require direct Replicate/Stability API integration
- Run `rake ruby_llm_agents:refresh_image_pricing` to update cached pricing data
