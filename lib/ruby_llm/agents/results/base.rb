...
module RubyLLM
  module Agents
    ...
    class Result
      ...
      # @!group Caching
      # @!attribute [r] cached_at
      #   @return [Time, nil] When this response was first produced, if it was
      #     replayed from the response cache rather than generated now
      ...
      #
      # Used by the Cache middleware on a hit. The execution_id is re-pointed at
      # the row being written for THIS call — replaying the original's id made
      # the result impossible to correlate with the execution it produced.
      #
